...
static int process_packet(queue_context *context, uint32_t id, uint32_t incoming_mark,
  ...
  uint32_t verdict_mark = path | worker->config->provisional_bit;
  ...
  if (packet.protocol == IPPROTO_TCP && new_payload) {
  ...
  uint32_t budget = packet.protocol == IPPROTO_TCP
  ...
  if (created && identified && terminal &&
      sdwan_ndpi_confidence_authoritative(flow->confidence)) {
    flow->desired_path = sdwan_policy_choose(
        worker->policy, flow->application, flow->category_name, flow->active_path);
  ...
  if (terminal) {
    ...
    verdict_mark = flow->active_path | worker->config->terminal_bit;
    terminal_event(worker, flow, now);
  } else {
    verdict_mark = flow->active_path | worker->config->provisional_bit;
  }
  return send_verdict(context, id, NF_ACCEPT, verdict_mark);
}
