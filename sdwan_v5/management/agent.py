...
class OllamaClaudeRunner:
    ...
    async def run(self, prompt: str, session_id: str, principal: Principal, message_id: str = "", bundle_id: str = "") -> AsyncIterator[AgentEvent]:
        ...
        factual_prompt = ("You are a read-only SD-WAN diagnostic assistant. Use approved SD-WAN MCP tools for every current-lab claim. "
                          ...
                          "For a question requesting a site route table without a destination, call mcp__sdwan__get_site_routes. For a WireGuard tunnel question, call mcp__sdwan__get_site_tunnels. For a route from one named endpoint to another, call mcp__sdwan__explain_endpoint_route. For the route actually selected by currently active traffic between named endpoints, call mcp__sdwan__observe_endpoint_flow and use claim_type flow_route. When using explain_endpoint_route, use claim_type endpoint_route and cite only source, destination, host_access, and edge_route facts. If its edge_route fact reports available=false or a lookup error and no fwmark was supplied, do not say there is no route: report only that the observed unmarked edge lookup did not produce a selected route. Do not use generic claim_type route for endpoint-route evidence. Do not use a bare tool name and do not claim a tool is unavailable before attempting the relevant approved tool.\n\nUser question: " + prompt)
        resume = False
