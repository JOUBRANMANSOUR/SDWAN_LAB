...
class OllamaClaudeRunner:
    ...
    async def run(self, prompt: str, session_id: str, principal: Principal, message_id: str = "", bundle_id: str = "") -> AsyncIterator[AgentEvent]:
        ...
        factual_prompt = ("You are a read-only SD-WAN diagnostic assistant. Use approved SD-WAN MCP tools for every current-lab claim. "
                          ...
                          "For a question requesting a site route table without a destination, call mcp__sdwan__get_site_routes. For a WireGuard tunnel question, call mcp__sdwan__get_site_tunnels. Do not use a bare tool name and do not claim a tool is unavailable before attempting the relevant approved tool.\n\nUser question: " + prompt)
        resume = False
