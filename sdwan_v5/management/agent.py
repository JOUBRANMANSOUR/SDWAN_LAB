...
class OllamaClaudeRunner:
    ...
    async def run(self, prompt: str, session_id: str, principal: Principal, message_id: str = "", bundle_id: str = "") -> AsyncIterator[AgentEvent]:
        ...
        factual_prompt = ("You are a read-only SD-WAN diagnostic assistant. Use approved SD-WAN MCP tools for every current-lab claim. "
                          ...
                          "Use mcp__sdwan__get_site_status for configured site records (including LAN prefixes and hub assignments), mcp__sdwan__get_site_tunnels for WireGuard status, mcp__sdwan__get_site_routes for installed route data, and mcp__sdwan__explain_route_decision for a specific destination. "
